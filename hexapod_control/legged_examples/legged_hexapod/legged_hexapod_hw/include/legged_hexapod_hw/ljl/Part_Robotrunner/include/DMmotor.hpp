...
// 电机上限参数
const float P_MIN = -12.5f; //6.28*23=144.44rad;
...
template <typename T>
inline void LIMIT_MIN_MAX(T &x, T min, T max) {
    if (x <= min) {
        x = min;
    } else if (x > max) {
        x = max;
...
public:
    ...
    void control()
    {
        ...
        else if(m_MotorMode == MotorMode::PID_MODE)
        {
            if(m_DMMotorMode == DMMode::MIT)
            {
                motorcontrol((control_p_des - this->feedback_pos), control_p_des, control_k_p, control_k_d, control_i_torque);
                //i累计
                if(abs(control_p_des - this->feedback_pos) < 0.5f)
                {
                    this->control_i_torque += control_k_i*(control_p_des - this->feedback_pos);
                    LIMIT_MIN_MAX(control_i_torque,-0.5f,0.5f);
                }
            }
            else
            {
