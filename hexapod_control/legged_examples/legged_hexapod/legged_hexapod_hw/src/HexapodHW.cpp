...
namespace legged
{
    ...
    bool HexapodHW::initWithProject(ros::NodeHandle &root_nh, ros::NodeHandle &robot_hw_nh, IrisTac_Project& Project)
    {
        project_ = &Project;
        return init(root_nh, robot_hw_nh);
    }
    ...
    void HexapodHW::read(const ros::Time &time, const ros::Duration & /*period*/)
    {
        ...
        if (jointStateReceived_)
        {
            ...
            {
                if (project_) {
                    jointStateMsg_.position[3*i]=project_->m_hexpodController.m_LegMotor[3*i].feedback_pos;
                    jointStateMsg_.position[3*i+1]=project_->m_hexpodController.m_LegMotor[3*i+1].feedback_pos;
                    jointStateMsg_.position[3*i+2]=project_->m_hexpodController.m_LegMotor[3*i+2].feedback_pos;
                    jointStateMsg_.velocity[3*i]=project_->m_hexpodController.m_LegMotor[3*i].feedback_vel;
                    jointStateMsg_.velocity[3*i+1]=-project_->m_hexpodController.m_LegMotor[3*i+1].feedback_vel;
                    jointStateMsg_.velocity[3*i+2]=project_->m_hexpodController.m_LegMotor[3*i+2].feedback_vel;
                    jointStateMsg_.effort[3*i]=project_->m_hexpodController.m_LegMotor[3*i].feedback_torque;
                    jointStateMsg_.effort[3*i+1]=-project_->m_hexpodController.m_LegMotor[3*i+1].feedback_torque;
                    jointStateMsg_.effort[3*i+2]=project_->m_hexpodController.m_LegMotor[3*i+2].feedback_torque;

                } else {
                    // 提供默认值或错误处理
                    jointStateMsg_.position[3*i] = 0.0;
                    jointStateMsg_.position[3*i+1] = 0.0;
                    jointStateMsg_.position[3*i+2] = 0.0;
                }
